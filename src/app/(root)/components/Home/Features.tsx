      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="bg-surface-light dark:bg-surface-dark p-6 rounded-lg shadow-light dark:shadow-dark">
              <feature.icon className="w-12 h-12 text-primary dark:text-primary-dark mb-4" />
              <h3 className="text-h3 text-textPrimary-light dark:text-textPrimary-dark mb-2">{feature.title}</h3>
              <p className="text-bodySmall text-textSecondary-light dark:text-textSecondary-dark">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
